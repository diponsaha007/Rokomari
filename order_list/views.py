...
def get_order(user_id):
    result = conn.cursor()
    result.execute(
        "SELECT ORDER_ID , ORDER_DATE,RECEIVED_DATE,ORDER_LOCATION , TOTAL_PRICE from ORDER_LIST WHERE USER_ID = :bv1 ORDER BY RECEIVED_DATE DESC NULLS FIRST, ORDER_DATE DESC",
        bv1=user_id)
    li = []
    while True:
        cnt = result.fetchone()
        ...
        result2 = conn.cursor()
        result2.execute(
            "SELECT O.BOOK_ID, B.BOOK_NAME ,A.AUTHOR_NAME, O.QUANTITY FROM BOOK B JOIN ORDER_DETAILS O On (B.BOOK_ID = O.BOOK_ID) JOIN AUTHOR A ON(A.AUTHOR_ID=B.AUTHOR_ID) WHERE O.ORDER_ID = :bv1",
            bv1=order_id)
        while True:
